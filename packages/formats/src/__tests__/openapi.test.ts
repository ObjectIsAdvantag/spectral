...
describe('OpenAPI format', () => {
  describe('OpenAPI 2.0 aka Swagger', () => {
    it.concurrent.each(['2.0.0', '2', '2.0'])('recognizes %s version correctly', version => {
  ...
  describe('OpenAPI 3.x', () => {
    it.concurrent.each(['3.0.0', '3', '3.0', '3.1.0', '3.0.3', '3.2'])('recognizes %s version correctly', version => {
  ...
  describe('OpenAPI 3.0', () => {
    it.concurrent.each(['3.0.0', '3.0', '3.0.3'])('recognizes %s version correctly', version => {
      expect(oas3_0({ openapi: version }, null)).toBe(true);
    });

    it.concurrent.each(['3', '3.1', '3.1.0', '3.1.3'])('does not recognize %s version', version => {
      expect(oas3_0({ openapi: version }, null)).toBe(false);
    });
  });

  describe('OpenAPI 3.1', () => {
    it.concurrent.each(['3.1.0', '3.1', '3.1.1'])('recognizes %s version correctly', version => {
      expect(oas3_1({ openapi: version }, null)).toBe(true);
    });

    it.concurrent.each(['3', '3.0', '3.0.3', '3.0.0'])('does not recognize %s version', version => {
